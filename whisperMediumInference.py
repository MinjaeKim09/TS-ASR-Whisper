#!/usr/bin/env python3
"""
Whisper Medium Inference Script
Runs evaluation on regular Whisper medium model using LibriMix dataset
"""
...
def main():
    """Run inference/evaluation on regular Whisper medium model using LibriMix dataset."""
    ...
    # Print status
    print("=" * 60)
    print("Whisper Medium Inference Script")
    print("=" * 60)
    print("Running evaluation on regular Whisper medium model...")
    print("Using LibriMix dataset (same as librimixMedium.py)")
    ...
    config_file = config_dir / "whisper_medium_librimix_inference.yaml"
    
    # Create inference configuration
    config_content = """# @package _global_

experiment: "whisper_medium_librimix_inference"
...
"""
    ...
    cmd = [
        python_cmd,
        str(main_script),
        "+decode=whisper_medium_librimix_inference"
    ]
    
    try:
        print(f"Executing command: {' '.join(cmd)}")
        print("=" * 60)
        print("🚀 Starting Whisper medium inference...")
        print("This will evaluate the regular Whisper medium model on LibriMix test set.")
        print("=" * 60)
        ...
        if result.returncode == 0:
            print("✅ Whisper medium inference completed successfully!")
            print(f"Check the output directory: {script_dir}/exp/whisper_medium_librimix_inference/")
            print("Results and predictions are saved there.")
        else:
            print(f"❌ Whisper medium inference failed with return code: {result.returncode}")
